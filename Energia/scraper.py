...
# scraper.py
# ----------
# Módulo ScraperEngine: responsável pela coleta de dados dos 18 veículos.
#
...
#      capturar a pauta geral do setor elétrico na janela de tempo.
#   2) TARGETED SEARCH (BUSCA INTERNA/"LUPA"): para portais com busca
#      server-side (WordPress), consulta o endpoint de busca pelas 7 entidades
...
class ScraperEngine:
    ...
    def _eh_titulo_lixo(titulo):
        ...
        return any(p in t for p in padrões_lixo)
    ...
    def _coletar_feed_unico(self, nome_veiculo, rss_url):
        ...
        try:
            ...
            for entry in feed.entries:
                ...
                if self._eh_titulo_lixo(titulo):
    ...
    def _coletar_via_google_news_termo(self, nome_veiculo, dominio):
        ...
        para cada termo crítico:
          a) As 7 entidades-alvo (ABRADEE, ABIAPE, ABEEólica, ABRAGE, ABiogás,
             Renova Energia, ABRATE).
          b) Termos regulatórios de alta relevância (Leilão de Reserva,
             Bandeira Tarifária, Curtailment, ANEEL).
    ...
    def _busca_html(self, nome_veiculo, url_busca_html):
        ...
        try:
            ...
            for artigo in soup.find_all("article"):
                ...
                if not titulo or self._eh_titulo_lixo(titulo):
                    continue
