...
# main.py
# -------
# PONTO DE ENTRADA do "Motor de Busca e Clipping de Notícias de Mercado de Capitais".
#
# Fluxo completo (pipeline):
#   1.  CLI interativo: pergunta quantas horas passadas devem ser varridas.
#   2.  ScraperEngine coleta as notícias dos veículos (via RSS com
...
def _configurar_log_em_arquivo():
    ...
    execução em <Resumos diários - ABRASCA>/Logs/execucao.log.
...
def ler_argumentos_janela():
    ...
    parser = argparse.ArgumentParser(
        description="Motor de Busca e Clipping de Notícias de Mercado de Capitais"
    )
    parser.add_argument(
        "--horas",
        type=int,
...
def pre_filtrar_rapido(noticias, filtro):
    ...
    PRINCÍPIO (precisão por veículo):
      - VEÍCULOS DEDICADOS AO MERCADO DE CAPITAIS (config.VEICULOS_DEDICADOS):
      ...
      - MÍDIA GERAL (Folha, Estadão, Valor, Poder 360 etc.): além dos sinais
        ...
        de ENTRADA é ALARGADO (config.TERMOS_ENTRADA_MIDIA_GERAL: "bolsa",
        "ações", "CVM", "IPO", "governança", "aciônista"..."):
    ...
    for noticia in noticias:
        ...
        # Veículos DEDICADOS ao mercado de capitais: nunca pré-descartar (nem
        # pela blacklist do pré-filtro). A matriz completa decide tudo.
        if veiculo in config.VEICULOS_DEDICADOS:
        ...
        # (apenas para mídia geral; nos veículos dedicados a matriz decide).
        ...
        # (Versão PERMISSIVA do N2: qualquer chave forte/macro/estrita mantém
        # a matéria; a âncora de mercado é exigida só na avaliação completa.)
        tem_cliente = bool(filtro._nivel1_clientes(normalizado_pre))
        tem_nivel2 = filtro._n2_qualquer_chave(normalizado_pre)
...
def avaliar_candidata(motor_engine, filtro, noticia):
    ...
    # Superfície editorial (título + resumo), usada pelo Nível 2 como fonte
    # adicional de âncora quando avaliando o full-text.
    ...
    # Porta de saída BRASIL (gate GLOBAL do pipeline): a ABRASCA acompanha
    # companhias ABERTAS BRASILEIRAS. Uma matéria APROVADA na matriz semântica
    # (full-text) é REPROVADA aqui quando o TÍTULO + RESUMO não citam o
    # mercado brasileiro (B3, Ibovespa, CVM, real, empresa brasileira...) E
    # a superfície carrega sinal de ORIGEM ESTRANGEIRA (Nasdaq/NYSE, país, SEC
    # ou companhia estrangeira: SpaceX, Shein, Nvidia, LVMH, Novartis...).
    # Assim, notícia sobre empresa brasileira continua passando mesmo sem a
    # palavra "Brasil" (ex.: "Vale nega estudos sobre IPO"), agora o noticiário
    # estrangeiro fica fora. Remove a assinatura "Investing.com Brasil" que o
    # Google News costuma colar no resumo — senão "Brasil" bateria em qualquer
    # matéria estrangeira.
    superficie_normalizada = filtro._normalizar(noticia.get("titulo") or superficie)
    superficie_sem_assinatura = re.sub(
        r"investing\.com\s*brasil|^\s*br\s*investing\b|\bfinan[çc]as,\s*c[âa]mbio e",
        " ",
        superficie_normalizada,
    )
    veiculo_lower = (noticia.get("veiculo") or "").lower()
    if is_relevante:
        # EXCEÇÃO: uma companhia BRASILEIRA nomeada no TÍTULO
        # (config.COMPANHIAS_ANCORA_BRASILEIRAS) exime a matéria do gate —
        # ex.: "Banco Inter desembarca na Argentina" é notícia de companhia
        # brasileira, não estrangeira.
        citou_companhia_brasileira = False
        for nome in config.COMPANHIAS_ANCORA_BRASILEIRAS:
            if filtro._contem(
                superficie_sem_assinatura, filtro._normalizar(nome)
            ):
                citou_companhia_brasileira = True
                break

        # REGRA A — Veículos ESTRANGEIROS (ex.: BR Investing / Investing.com
        # Brasil): a decisão do cliente é "SÓ relevante p/ Brasil". O título
        # tem de citar o mercado brasileiro OU uma companhia brasileira; o
        # noticiário Reuters sobre Nasdaq, juros dos EUA, Europa, Ásia etc.
        # não interessa mesmo passando na matriz.
        if "investing" in veiculo_lower:
            if not (citou_companhia_brasileira or
                    filtro._tem_ancora_brasil(superficie_sem_assinatura)):
                logger.debug(
                    "BR Investing sem âncora Brasil no título (reprovado): %s",
                    noticia.get("titulo", "")[:120],
                )
                is_relevante = False
                noticia["is_relevante"] = False

        # REGRA B — Veículos BRASILEIROS: reprova APROVADAS cujo TÍTULO marca
        # origem ESTRANGEIRA (Nasdaq/NYSE, país, SEC, companhia estrangeira
        # conhecida: SpaceX, Shein, Nvidia, LVMH, Novartis...). O TÍTULO é o
        # sinal do assunto; o resumo costuma citar NYSE/Nasdaq de passagem em
        # boas matérias brasileiras. Só escapa citando companhia brasileira.
        elif not citou_companhia_brasileira:
            if filtro._tem_origem_estrangeira(superficie_sem_assinatura):
                logger.debug(
                    "Título com origem estrangeira sem companhia brasileira "
                    "nomeada (reprovado): %s",
                    noticia.get("titulo", "")[:120],
                )
                is_relevante = False
